...
public:
	...
	virtual void Tick(const float TimeDelta) override
	{
		...
		if (!bWaitingOnFPSChart && !bWaitingOnFinalizingVideo)
		{
			...
			{
#if WITH_EDITOR
				if (Controller->ShouldRestartPieEachIteration())
				{
					GUnrealEd->RequestEndPlayMap();
					Controller->SetTestState(ECitySampleAutoTestState::RestartingPIE);
				}
				else
#endif
				{
...
// Restarting
class FCitySampleAutoTestState_RestartingPIE : public FCitySampleAutoTestState
{
public:
	FCitySampleAutoTestState_RestartingPIE(UCitySampleTestControllerAutoTest* const TestController) : FCitySampleAutoTestState(TestController) {}
...
#if WITH_EDITOR
		ShutdownPIEDelegate = FEditorDelegates::ShutdownPIE.AddLambda([this](bool)
			{
				FTSTicker::GetCoreTicker().AddTicker(FTickerDelegate::CreateLambda([](float)->bool
					{
						...
						return false;
					}), static_cast<float>(Controller->GetSecondsBetweenIterationsPIE()));
			});
#endif
	}
	...
	{
#if WITH_EDITOR
		FEditorDelegates::ShutdownPIE.Remove(ShutdownPIEDelegate);
		ShutdownPIEDelegate.Reset();
#endif
		Super::End(NewState);
	}

private:
	FDelegateHandle ShutdownPIEDelegate;
};
...
void UCitySampleTestControllerAutoTest::OnInit()
{
	...
	States[(uint8)ECitySampleAutoTestState::RestartingPIE] = new FCitySampleAutoTestState_RestartingPIE(this);
	...
	StateTimeouts[(uint8)ECitySampleAutoTestState::RestartingPIE] = 300.f;
...
#if WITH_EDITOR
	FEditorDelegates::ShutdownPIE.AddWeakLambda(this, [this](bool)
		{
			TRACE_BOOKMARK(TEXT("PIE Stopping - Iteration %d"), CompletedPieIterations);
			CompletedPieIterations++;
		});

	FEditorDelegates::BeginPIE.AddWeakLambda(this, [this](bool)
		{
			TRACE_BOOKMARK(TEXT("PIE Starting - Iteration %d"), CompletedPieIterations);
		});
#endif
}
